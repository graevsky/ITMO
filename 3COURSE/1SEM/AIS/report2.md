...
- Провести предварительную обработку, разделение данных.
...
Задача работы — использование метода ближайших соседей (kNN) для предсказания вероятности наличия диабета у пациентов на основе набора медицинских данных, таких как уровень глюкозы, ИМТ , возраст и другие. Целевая переменная — столбец "Outcome", где значение 1 означает наличие диабета, а 0 — его отсутствие.

Цели:
- Произвести предобработку данных (масштабирование признаков и очистка от пропущенных значений).
- Виузализировать (в том числе и в 3д) статистику по распределению параметров датасета
...
- Добавить метрику успешности студентов на основе их оценки.
- Выбрать $\sqrt{n}$ случайных признаков.
...
- Оценка качества разбиений: Для оценки качества разбиений используются метрики, такие как:
  ...
  - Индекс Gini — мера "чистоты" группы объектов.
...
На выходе дерево решений и предсказания
```
