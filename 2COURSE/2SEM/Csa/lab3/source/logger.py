...
def save_as_yaml(source_file, input_file, output_yaml):
    ...
    (output, instr_count, ticks,
     execution_log) = run_simulation_and_capture_output(
        machine_code, input_data
    )
    ...
    data = {
        ...
        "out_code": LiteralScalarString("\n".join(f"{item}" for
                                                  item in machine_code)),
        "out_stdout": LiteralScalarString(
            f"Source instr: {len(machine_code)}"
            f"\n============================================================\n"
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python logger.py "
              "<source_file> <input_file> <output_yaml>")
    else:
